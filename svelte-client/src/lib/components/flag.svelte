<script lang="ts">
 ...
 onMount(() => {
   ...
   loader.load(
     '/lana-gliding.glb', // Update this to your actual .glb path
     (gltf) => {
       ...
       if (gltf.animations && gltf.animations.length > 0) {
        ...
        //  const clip = gltf.animations[1]; // use the first available animation
       ...
       const animate = () => {
         ...
         if (model) {
          ...
          const speed = 0.2; // Units per second along X-axis
          model.position.x += speed * delta;

          // Looping Movement
          const loopBoundary = 2; // Adjust based on your scene
          if (model.position.x > (loopBoundary + 1.2)) {
            model.position.x = -loopBoundary;
          }
        }
